...
export default createApiHandler(async (req, res) => {
  ...
  const data = await runLobbyTransaction(getDatabase().ref(`lobbies/${lobbyCode}`), (lobby) => {
    ...
    if (player.role === 'agent') return commitLobby(lobby, { idempotent: true });
    if (Object.values(lobby.players).some((candidate) => candidate.role === 'agent' && !candidate.eliminated)) {
      return rejectLobby(409, 'AGENT_ALREADY_CLAIMED', 'Agent role is already claimed');
    }
    return commitLobby({
      ...lobby,
      players: { ...lobby.players, [uid]: { ...player, role: 'agent' } },
      lastActivityAt: now
    }, { idempotent: false });
  });
